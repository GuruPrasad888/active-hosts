...
def log_device_info_add(device, json_file):
    ...
    existing_entry = None
    for entry in data['Active Devices']:
        if entry['IP Address'] == device['IP Address'] and entry['MAC Address'] == device['MAC Address']:
    ...
    if existing_entry:
        ...
        log_message = f"{device.get('IP Address', '')}, {device.get('MAC Address', '')}, {device.get('Device Name', '')}, updated"
    else:
        # Add a new entry
